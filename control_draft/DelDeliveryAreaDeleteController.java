...
public class DelDeliveryAreaDeleteController implements ActionListener{
	
	private DTO arrays;
	private RestaurantBean restoBean;
	private RestaurantDAO restoDAO;
	private RestaurantDeleteView view;
	private RestaurauntModel model;
	
	
	
	public DelDeliveryAreaDeleteController(RestaurauntModel model, RestaurantDeleteView view) {
		this.model = model;
		this.view = view;
	...
	public void actionPerformed(ActionEvent e) {		
		...
		for (int i = 0; i < view.getList_1().getModel().getSize(); i++) {
			if(i != view.getList_1().getSelectedIndex()) {
				String item = (String) view.getList_1().getModel().getElementAt(i);
				model.addElement(item);  
			}
		}
		view.getList_1().setModel(model);
		
		
		RestaurantBean newRB = new RestaurantBean(view.getTxtfield_name().getText(), view.getTxtfield_address().getText(), view.getList_1().getModel(), view.getTxtfield_telephone().getText());
		...
		RestaurantBean oldRB = (RestaurantBean) view.getList().getSelectedValue();
		...
		view.getList().setModel(modelResto);		
