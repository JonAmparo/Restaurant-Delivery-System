...
public class AddMenuController implements ActionListener {
	...
	private RestaurantCreateMenuView view;
	private RestaurauntModel model;

	public AddMenuController(RestaurantCreateMenuView view, RestaurauntModel model) {
		this.view = view;
		this.model = model;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (view.getItemTxt_field().getText().equals("") || view.getPriceTxt_field().getText().equals("")) {
			...
			if (view.getMenuItems_list().getModel().getSize() > 0) {
				...
				RestaurantMenuBean restoMenuBean = new RestaurantMenuBean(view.getItemTxt_field().getText(),
						Integer.parseInt(view.getPriceTxt_field().getText()));

				DefaultListModel<RestaurantMenuBean> model = (DefaultListModel<RestaurantMenuBean>) view
						.getMenuItems_list().getModel();
